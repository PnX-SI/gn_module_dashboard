...
export class DashboardHistogramComponent implements OnInit {
  ...
  getCurrentTax(event) {
    let searchForm = Object.assign({}, this.histForm.value);
    ...
    if (this.filter == 'Rechercher un taxon/une espèce...') {
      ...
      searchForm['taxon'] = event.item.cd_ref;
    } else {
    ...
    //// Sinon...
      ...
      // Accès aux données de la VM vm_synthese
      this.subscription = this.dataService.getDataSynthese(searchForm).subscribe(
        (data) => {
